...
migrations:
	docker compose -f ${PATH_DOCKER_COMPOSE} -p ${NAME} exec ${NAME_BACKEND} python manage.py makemigrations
	docker compose -f ${PATH_DOCKER_COMPOSE} -p ${NAME} exec ${NAME_BACKEND} python manage.py migrate

migrate:
	docker compose -f ${PATH_DOCKER_COMPOSE} -p ${NAME} exec ${NAME_BACKEND} python manage.py migrate

collectstatic:
	docker compose -f ${PATH_DOCKER_COMPOSE} -p ${NAME} exec ${NAME_BACKEND} python manage.py collectstatic --noinput
